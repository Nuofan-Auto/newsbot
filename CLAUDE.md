...
**Testing limitation**: `search_related()` requires `SERPER_API_KEY` in `.env` — returns `[]` if missing. LLM prompt/parser logic can be tested independently with fake `search_results` dicts.
...
Web search (optional — `/search` and `/explore` degrade gracefully to LLM-only without this):
- `SERPER_API_KEY` — Serper.dev API key; 2,500 free queries/month, no credit card required
...
RSS Feeds → RSSFetcher (lang per source) → NewsPipeline._enrich() → ArticleStore (SQLite cache)
                             ...
                             NewsBot: /news → zh block / en block
                                      ...
                                      /search <topic> → Serper.dev search + LLM synthesis
```
...
| `scrapers/web_search.py` | Serper.dev Google Search API; `search_related(query, n)` returns `list[dict]` with `{"title": str, "snippet": str, "url": str, "display_url": str}`; uses `tbs=qdr:w` to bias toward past-week results; returns `[]` if `SERPER_API_KEY` not set |
...
**GLMProvider**: uses `anthropic` SDK with `base_url="https://open.bigmodel.cn/api/anthropic"`. `max_tokens=2048`.

**ClaudeProvider**: uses `anthropic` SDK with no `base_url`. `max_tokens=2048`.

**MiniMaxProvider**: uses raw `requests` HTTP with 3-attempt retry loop and `base_resp.status_code` / `output_sensitive` checks. `max_tokens=2048`. Raises `RuntimeError` on content-moderation blocks (`output_sensitive=True`) so FallbackProvider can continue to the next provider.
...
Pass `extra_sources` dict to `CredibilityAnalyzer.__init__()`, or call `load_sources()` at runtime. Built-in entries: BBC News, Reuters, TechCrunch, CNN, Fox News, 36氪 (score=7), 少数派 (score=7), 澎湃新闻 (score=7, LEFT-CENTER), 机器之心 (score=7, CENTER). Unknown sources return a plain dict with score=5 — always guard with `hasattr(rating, "credibility_score")`.
...
| `/explore <n>` | `_explore()` | n = serial number from most recent `/news` call. Looks up `_session_articles[chat_id][n-1]`. Runs Serper.dev search + LLM via `provider._call_api(build_explore_prompt(...))`. |
| `/search <topic>` | `_search()` | Multi-word topic supported (joins `context.args`). `_detect_lang(topic)` auto-detects zh/en. Runs Serper.dev search + LLM synthesis. |
...
🔗 相关资讯/Related Sources
  • [Title](url): snippet  ← clickable link when URL available from Serper.dev
...
🗣️ 多方观点/Perspectives
  ...
  • [AI] perspective 2
```
In Telegram MarkdownV2 `[label](url)` links: only `label` passes through `_escape()`; `url` is used raw (same pattern as `🔗 阅读全文` links in `/news`). Serper.dev returns direct URLs in `items[].link`.
